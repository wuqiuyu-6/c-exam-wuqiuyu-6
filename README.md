...
注意事项
在 src/ 下编写代码，源文件自动被包含
头文件放入 include/ 目录
清理构建：cmake --build . --target clean
不要修改 test/ 和 scripts/test/ 下的测试文件
队列作业要求
数据结构
使用链表实现链式队列，包含以下结构：
...
特性要求
FIFO 特性：先进先出，入队顺序与出队顺序一致
容量限制：支持设置最大容量（capacity=0 表示无限制）
内存安全：正确处理 NULL 指针，销毁时释放所有节点
边界处理：空队出队/获取元素时返回 false/0
运行测试
# 构建并运行测试
cmake -B build
cmake --build build
